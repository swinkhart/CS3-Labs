...
hashmap<Key, Value, Compare, Hash>::hashmap(
    const Compare &comp, const Hash &hash) : size_(0), comp_(comp), hash_(hash)
{
...
hashmap<Key, Value, Compare, Hash>::erase(const Key &x)
{
    ...
    if (it != elems_[bucket].end()) // the element exists, erase it
    {
        ...
        {
            int nextBkt = bucket + 1;
            if (nextBkt >= hash_.numBuckets())
            {
                nextBkt = 0;
            }

            while (elems_[nextBkt].empty())
            {
                ++nextBkt;
            }
            return make_pair(&(*elems_[nextBkt].begin()), true);
        }
    }
    else
    {
...
{
    auto pIndex = insert(make_pair(x, Value()));
    return pIndex.first->second;
}
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n)
{
    ...
    for (auto &index : tmp)
    {
        size_t item = hash_.hash(index.first);
        elems_[item].push_back(index);
    }
}